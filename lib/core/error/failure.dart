...
import 'package:primedehealth/core/constants/strings.dart';
...
class ConvertFailureToString {
  ...
  String call(Failure failure) {
    return failure.map(
      app: (value) => value.message ?? Strings.appFailure,
      serverError: (failure) => failure.message,
      noInternet: (failure) => Strings.noInternet,
      unknown: (failure) => Strings.unknown,
    );
  }
}
